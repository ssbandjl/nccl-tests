...
# On 192.168.1.11
CUDA_VISIBLE_DEVICES=0 NCCL_RANK=0 NCCL_LOCAL_RANK=2 NCCL_WORLD_SIZE=4 NCCL_UNIQUE_ID=$(cat /root/project/ai/nccl-tests/uid) \
./build/all_reduce_perf -b 8 -e 1G -f 2 -g 1 &

CUDA_VISIBLE_DEVICES=1 NCCL_RANK=1 NCCL_LOCAL_RANK=3 NCCL_WORLD_SIZE=4 NCCL_UNIQUE_ID=$NCCL_UNIQUE_ID \
./build/all_reduce_perf -b 8 -e 1G -f 2 -g 1 &
