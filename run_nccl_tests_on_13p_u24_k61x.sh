...
# make network better cross nodes
NCCL_P2P_DISABLE=1
NCCL_SHM_DISABLE=1
...
for algo in ${ALGO_LIST};do
  ...
  mpirun -np 8 -H 192.168.1.10:4,192.168.1.11:4\
    ...
    --allow-run-as-root \
    /root/project/ai/nccl-tests/build/${algo} -b 4 -e 64 -w 30 -f 2 -c 0 -g 1 -n 100 > "$log_file" 2>&1
  ...
  #   -x NCCL_TOPO_DUMP_FILE=topo_${algo}.xml \
  #   --allow-run-as-root \
  #   /root/project/ai/nccl-tests/build/${algo} -b 16M -e 16M -w 30 -f 2 -c 0 -g 1 -n 100 >> "$log_file" 2>&1
done
